...
export async function updateSession(request: NextRequest) {
  ...
  if (!supabaseUrl || !supabaseAnonKey || supabaseUrl === 'your_supabase_project_url' || supabaseAnonKey === 'your_supabase_anon_key') {
    // Return early if Supabase is not configured - don't try to create client
  ...
  // IMPORTANT: getUser() verifies the JWT is valid, but doesn't refresh it
  ...
  console.log('Middleware - checking user for path:', request.nextUrl.pathname, 'user:', user?.email || 'none')
